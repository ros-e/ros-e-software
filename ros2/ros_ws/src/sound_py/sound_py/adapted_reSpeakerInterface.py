...
"""
This file contains the interface class to communicate with a respeaker module. 
"""
...
class RespeakerInterface(object):
    ...
    def __init__(self):
        ...
        self.get_logger().info("Initializing Respeaker device")
        self.dev.reset()
        self.pixel_ring = usb_pixel_ring_v2.PixelRing(self.dev)
        self.set_led_think()
        time.sleep(5)  # it will take 5 seconds to re-recognize as audio device
        self.set_led_trace()
        self.get_logger().info("Respeaker device initialized (Version: %s)" % self.version)
