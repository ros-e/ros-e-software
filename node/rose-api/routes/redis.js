...
router.get('/', function (req, res) {
    ...
    let resJson = {errors: [], key: key };

    if (key != undefined) {
        r.client.get(key, (err, reply) => {
            if (err != null) resJson.errors.push(err);
            resJson.value = reply;
            res.json(resJson);
        });
    } else {
        resJson.errors.push("Key is undefined")
        res.json(resJson);
    }
...
router.post('/', jsonParser, function (req, res) {
    ...
    let resJson = {errors: [], key: key, value: value };

    if (key != undefined && value != undefined) {
        resJson.success = r.client.set(key, value);
    } else resJson.errors.push("Key or value are undefined")
